...
function collectPlurals(strings) {
  return strings.filter((string) => string.at(-1) === 's');
}
...
function createArray(count, char) {
...
function countTheObjects(arr) {
  return arr.reduce((count, item) => {
    if (typeof item === 'object') return count + 1;
...
function getEmptyTills(tills) {
  return tills.filter((till) => !till.checkout.length);
}
