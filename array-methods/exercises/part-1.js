...
runTest("get the multiples of 10 from 10 to 100", function () {
  ...
  const multiplesOf10 = items.map((num) => num * 10);
...
runTest("can get all the words ending in er", function () {
  ...
  const wordsEndingInEr = words.filter((word) => word.substring(word.length - 2) === 'er');
...
runTest("can get the names of staff over 28", function () {
  ...
  const staffOver28 = staff.filter((person) => person.age > 28);

  const namesOfStaffOver28 = staffOver28.map((person) => person.name);
